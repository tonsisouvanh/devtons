...
const ProjectCard = ({ id, name, desc, imgs, techStack, github, website }) => {
  ...
  return (
    <>
      ...
      >
        <img
          className="w-full h-full object-cover transition-transform ease-in-out duration-150 group-hover:scale-110"
          src={
            imgs && imgs.length > 0 ? imgs[0] : "https://upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/330px-No-Image-Placeholder.svg.png?20200912122019"
          }
          alt="/"
        />
        ...
        <div className="absolute flex flex-col justify-between top-0 left-0 w-full h-full bg-gradient-to-t from-black">
            ...
            <div className="text-xl flex gap-3 p-2 bg-black/60 rounded-bl-md">
              <span>
                <GoEye
                  ...
                  className="cursor-pointer hover:scale-110 text-[#00FFFF]"
                />
              </span>
          ...
          <div className="p-3 text-sm space-y-2">
            <p className="text-gray-300 md:text-md">{desc}</p>
            {/* Teach stack */}
        ...
        <AnimatePresence>
          {select === id ? (
            ...
            >
              <CgCloseO
                className="text-3xl cursor-pointer opacity-75 absolute top-3 right-3 transition hover:opacity-100 md:top-5 md:right-8 md:text-4xl"
                onClick={() => setSelect(0)}
              />
              ...
              <div className=" max-w-[1240px] relative flex justify-center items-center md:max-w-[1300px]">
                <AiFillLeftCircle
                  onClick={prevSlide}
                  className="select-none text-[#00B0FF] absolute cursor-pointer text-3xl top-[50%] left-1 opacity-60 transition hover:opacity-100 md:text-5xl"
                />
                <AiFillRightCircle
                  onClick={nextSlide}
                  className="select-none text-[#00B0FF] absolute cursor-pointer text-3xl top-[50%] right-1 opacity-60 transition hover:opacity-100 md:text-5xl"
                />
